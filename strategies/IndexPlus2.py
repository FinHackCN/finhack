        ...
        for index,row in sr.iterrows():
            ...
            if symbol in idx_weight.index.values:
                if row['pred']<0.95:
                    ...
                    idx_weight.loc[symbol,'weight']=0
                    idx_weight.drop(index=symbol)
                else:
                    idx_weight.loc[symbol,'weight']=float(idx_weight.loc[symbol]['weight'])*math.pow(row['pred'],10)
