...
配置文件 `args.conf` 支持多层级配置，按以下优先级加载：
...
### 4.2 ClassLoader实现 (`finhack/library/class_loader.py`)
```python
class ClassLoader:
    @staticmethod
    def get_module(module_path='', user_module_path=''):
        ...
        if os.path.exists(user_module_path):
            ...
            module_name = filename[:-3] if filename.endswith(".py") else "user_module"
...
### 10.1 模块加载失败处理
- 如果系统模块不存在，自动回退到base_loader
- 如果用户模块语法错误，显示详细错误信息并退出
- 如果类不存在，显示可用类列表
